...
var SmeList = function(id, options) { 
	this._el = null;
	this._lists = null;
	this._export = null;
	...
	this._indexOfProject = function(number, limit) {
		
		return document.getElementById('project-' + number) != null;
	};
	...
	this.refresh = function(data, filters, terminate) { 
		...
		// unused for now.
		if(this._filters) {
			if(this._filters.fulltext) { this._selection.push('Matching "' + this._filters.fulltext + '".'); }
			if(this._filters.countries.length > 0) { this._selection.push('Located in ' + this._filters.countries.join(',') + '.'); }
			if(this._filters.phases.length > 0) { this._selection.push('In phase ' + this._filters.phases.join(' or ') + '.'); }
			//if(this._filters.topics.length > 0) { this._selection.push('Related to  ' + topicsId2Label(this._filters.topics).join(',') + '.'); }
			if(this._filters.cutoffs.length > 0) { this._selection.push('For call date ' + this._filters.cutoffs.join(' or ') + '.'); }
			if(this._filters.from) { this._selection.push('Ending after ' + this._filters.from + '.'); }
			if(this._filters.to) { this._selection.push('Ending before ' + this._filters.to + '.'); }
			if(this._filters.budgetMin) { this._selection.push('Minimum budget of ' + this._filters.budgetMin + '€.'); }
			if(this._filters.budgetMax) { this._selection.push('Maximum budget of ' + this._filters.budgetMax + '€.'); }
		}
		*/
		this._projectsCount = 0;
		...
		for(var i = 0; i < this._data.length; i++) { 
			...
			for(var p = 0; (p < data.projects.length); p++) {
				if(projects.indexOf(data.projects[p].number) === -1)  projects.push(data.projects[p].number);
			}
		}
	...
	this._searchArray = function(number, array) { 
	...
	this._displayResult = function() { 
		...
		for(var i = 0; i < this._data.length; i++) { 
			...
			for(var p = 0; (p < data.projects.length); p++) {
				...
				if(this._searchArray(project.number, projects) === false) { 
	...
	this._buildProject = function(project) { 
		...
		acronym.id = 'project-' + project.number;
		title.className = 'project-title';
		
		link = document.createElement('a');
		link.href = '#';
		link.className = 'project-button';
		link.title = link.innerHTML = project.acronym;
		link.setAttribute('data-id', project.number);
