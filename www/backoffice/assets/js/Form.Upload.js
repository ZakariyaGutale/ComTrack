...
Form.Upload = new Class({
	...
	initialize: function(input, options){
		...
		if(options.onRequest) this.onStart = options.onRequest;
	...
	modernUpload: function(input){
		...
		var progress = this.options.progress.setStyle('display', 'none');
	...
	submit: function(inputFiles, inputname, uploadReq){
		...
		if(Array.isArray(inputFiles.getFiles()))
			inputFiles.getFiles().each(function(file){
				uploadReq.append(inputname , file);
			});
		else
			uploadReq.append(inputname , inputFiles.getFiles());
		uploadReq.send();
	},
